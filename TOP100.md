...
class Solution:
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        def dfs(i,j):
            ...
            grid[i][j] = '2'
            for di,dj in directions:
                dfs(i+di,j+dj) #只要当前i和j是符合的就是true，不断dfs走是为了把同一块儿岛屿的标记为2
            return True
        output = 0
        for i in range(m):
            for j in range(n):
...
class Solution:
    def findNthDigit(self, n: int) -> int:
        digits = 1
        r = 9
        while n >= r*digits: #确定位数
            n -= r*digits
            r *= 10
            digits += 1
        value = 10**(digits-1)+n//digits #确定值
        #print(value,n)
        if n%digits == 0: 
            value -= 1
        #确定数字位置
...
> **A:** 将身高从高到低同时k从低到高排序，之后k就是遍历到这个人时他所处的数组中的位置，因为此时剩余的数中不会有比这个人高的了
```python
class Solution: #排序
...
贪心算法，按右区间排序，新的右区间只要满足左区间比前面的右区间大就行，因为自己的右区间都一样，注意这个题只是右区间有用
...
贪心算法，每次都挑当前糖果里比人胃口里最小的大的最小的糖果，故需要先排序
...
DFS，类似200，只要没走过就加1，但是要dfs更新掉连接的边即visited
...
滑动窗口问题，比76思路简单，一开始想复杂了，因为是全排列所以子串的长度一定要一样，固定长度的滑动窗口即可
...
class Solution:
    def checkInclusion(self, s1: str, s2: str) -> bool:
        ...
        if m > n: return False
        l = 0
        count = m #我用一个count来判断加速，也可以用26个字母映射的字典或者列表进行比较
        hashmap = {}
        for i in s1:
            hashmap[i] = hashmap.get(i,0)+1
        for j in range(m):
        ...
        l,r = 0,m
        while r < n:
            if s2[r] in hashmap:#r右移
                hashmap[s2[r]] -= 1
                if hashmap[s2[r]] >= 0:
                    count -= 1
            
            if s2[l] in hashmap:#l左移
                hashmap[s2[l]] += 1
                if hashmap[s2[l]] > 0:
                    count += 1
            if count == 0: return True#判断是否符合
            l,r = l+1,r+1
        return False               
```
...
class Solution:
    def maxAreaOfIsland(self, grid: List[List[int]]) -> int:
        ...
        directions = [(0,1),(1,0),(0,-1),(-1,0)]
        def dfs(i,j,area):
            if i < 0 or j < 0 or i > m-1 or j > n-1 or grid[i][j] != 1: #没办法走，边界and水and走过的地方
                return area
            grid[i][j] = 2 #走过的地方标记为2
            area += 1
            for di,dj in directions:
                area = dfs(i+di,j+dj,area) #四个方向继续走，每个方向积累的都算在area里面
            return area
        for i in range(m):
            for j in range(n):
...
从头开始遍历，不断扩大已有区间和当前s[i]的最后一个位置（表示需要走到的位置），当已有区间和最远距离重合时表示这一段符合要求了，记录长度
...
思路是left数组的最大值要小于right的最小值，找到第一个满足的left就return；可以一次遍历优化
...
#优化的方法，本质是right中的值不满足大于left的max时，left就要移动到这个点之后
class Solution:
    def partitionDisjoint(self, nums: List[int]) -> int:
        #left的最大值小于right的最小值
        n = len(nums)
        ...
        for i in range(n):
            ...
            if nums[i] < leftmax: #不符合要求了，要把分界点提到这里
                pos = i
                leftmax = allmax
        return pos+1
```

#### [[BFS]-1091. 二进制矩阵中的最短路径](https://leetcode-cn.com/problems/shortest-path-in-binary-matrix/)

经典BFS，用到了queue类似二叉树的BFS，图的DFS是递归方法比较简单
...
class Solution:
    def shortestPathBinaryMatrix(self, grid: List[List[int]]) -> int:
        ...
        if grid[0][0] == 1 or grid[n-1][n-1] == 1:
            ...
            return n
        #BFS是队列
        queue = [(0,0,1)] #i,j和路径长度
        grid[0][0] = 1
        while queue:
            i,j,depth = queue.pop(0)
            for di,dj in directions:
                newi,newj = i+di,j+dj
                if newi < 0 or newi > n-1 or newj < 0 or newj > n-1:
                    continue
                if grid[newi][newj] != 1:
