...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        # 可以先找到最小值index，然后根据target和nums[0]的关系分别比较
        # 一次遍历，每个mid划分出来一定有一段是完全升序的，分别比较即可
        ...
        def bi_search(nums,target):
            ...
            while l <= r:
                ...
                if nums[mid] < target:
                    ...
                    r = mid - 1
            return l
        index = bi_search(nums,target)
        if index > len(nums)-1 or nums[index] != target: return [-1,-1] #特殊情况判断，超过nums或者不在nums中
        return [index,bi_search(nums,target+1)-1] #找target+1的最左边index
```
    ...
    def combinationSum(self, candidates: List[int], target: int) -> List[List[int]]:
        ...
        def traceback(r,index,target):
            ...
            for i in range(index,n):
                ...
                traceback(r,i,target-candidates[i])
                r.pop()
        traceback([],0,target)
        return output
```
...
class Solution:
    def merge(self, intervals):
        ...
        i = 1
        while i < n:
            if intervals[i][0] <= output[-1][1] <= intervals[i][1]: #有重合且新的右边大
                output[-1][1] = intervals[i][1]
            elif intervals[i][0] > output[-1][1]: #无重合直接append
...
class Solution:
    def minPathSum(self, grid: List[List[int]]) -> int:
        #DP[i][j]表示grid[i-1][j-1]的最小路径，优化到1D空间
...
class Solution: #位运算，异或运算，a^a = 0，0^a=0，剩下的就是单个的
    def singleNumber(self, nums: List[int]) -> int:
        if not nums: return
        n = len(nums)
        output = 0
