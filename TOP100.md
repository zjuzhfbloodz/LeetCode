...
class Solution:
    def isMatch(self, s: str, p: str) -> bool:
        ...
        def matches(i: int, j: int) -> bool: #判断i和j两个位置的字符是否匹配
            if i == 0:
                return False
            if p[j - 1] == '.':
                return True
        ...
        f[0][0] = True
        for i in range(m + 1): #i必须从0开始，因为p='x*'时可以匹配空字符串；j则从1开始
    ...
    def combinationSum(self, candidates, target: int):
        ...
        for i in range(n):
            for j in range(candidates[i],target+1):
                if j == candidates[i]: dp[j] += [[j]]
                else: dp[j] += [x+[candidates[i]] for x in dp[j-candidates[i]]]
        return dp[-1]
```
...
class Solution:
    def trap(self, height) -> int:
        #第一思考，只有比周围都短才能接，接到最短的那个
        #第二思考，之后左边和右边都有比自己高（且是最高）的才能接，接的量是较小值
        #故需要对每个height[i]找到0-i-1的max和i+1-n-1的max
        n = len(height)
        if n < 3: return 0
        rain = 0
        dpl = [0 for i in range(n-2)]
        dpr = [0 for i in range(n-2)]
        dpl[0],dpr[0] = height[0],height[-1]
        for i in range(1,n-2): dpl[i] = max(height[i],dpl[i-1])
        for j in range(1,n-2): dpr[j] = max(height[n-j-1],dpr[j-1])
        for i in range(1,n-1):
            amount = min(dpl[i-1],dpr[n-i-2])
            if height[i] < amount:
                rain += amount - height[i]
        return rain
...
> **A:** 自己的想法是用动态规划去做，复杂度是O(N2)；可以用贪心算法做，思路是如果能走到某个index，**那么index之前的所有都可以走到**（关键点），因为是一步步走的不能跳跃
```python
class Solution: #DP
...
class Solution:
    def numTrees(self, n: int) -> int:
        #感觉是DP,DP[i] = DP[0]*DP[i-1]+(DP[1]*DP[i-2])+...+DP[i-1]*DP[0]
...
class Solution:
    def maxProduct(self, nums: List[int]) -> int:
        ...
        for i in range(1,n):
            if nums[i] >= 0:
                maxone = max(dp[i-1][0]*nums[i],nums[i])
                minone = min(dp[i-1][1]*nums[i],nums[i])
            else:
                maxone = max(dp[i-1][1]*nums[i],nums[i])
                minone = min(dp[i-1][0]*nums[i],nums[i])      
...
class Solution:
    def lengthOfLIS(self, nums: List[int]) -> int:
        if not nums: return
        n = len(nums)
#维护一个数组d，这个数组的第i个元素表示长度为i+1的最优序列的末尾元素（即最小的）
        que = [nums[0]]
        for i in range(1,n):
            #二分查找，找到第一个比nums[i]大的位置
            if nums[i] > que[-1]:
                que.append(nums[i])
                continue
            l,r = 0,len(que)-1
            while l < r:
                ...
                if que[mid] == nums[i]: 
                    l = mid
                    break
                elif que[mid] > nums[i]: r = mid - 1
                else: l = mid + 1
            if que[l] < nums[i]: que[l+1] = nums[i]
            else: que[l] = nums[i]
        return len(que)
...
class Solution:
    def longestCommonSubsequence(self, text1: str, text2: str) -> int:
        ...
        #dp[i][j]代表text1的前i-1个和text2的前j-1，思考好状态转移方程
        for p in range(m):
            for q in range(n):
                if text1[p] == text2[q]:
                    dp[p+1][q+1] = dp[p][q] + 1
